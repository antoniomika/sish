...
func init() {
	...
	rootCmd.PersistentFlags().BoolP("force-https", "", false, "Allow individual binds to request for https to be enforced")
	...
	rootCmd.PersistentFlags().IntP("log-to-file-max-size", "", 500, "The maximum size of outputted log files in megabytes")
	rootCmd.PersistentFlags().IntP("log-to-file-max-backups", "", 3, "The maximum number of rotated logs files to keep")
	rootCmd.PersistentFlags().IntP("log-to-file-max-age", "", 28, "The maximum number of days to store log output in a file")
